...
func (service serviceMessage) MarkAsReadBulk(
	ctx context.Context,
	request domainMessage.MarkAsReadBulkRequest) (response domainMessage.MarkAsReadBulkResponse,
	err error,
) {
...
func (service serviceMessage) MarkAsPlayed(
	ctx context.Context,
	request domainMessage.MarkAsPlayedRequest) (response domainMessage.GenericResponse,
	err error,
) {
	if err = validations.ValidateMarkAsRead(ctx, request); err != nil {
		return response, err
...
func (service serviceMessage) GetMedia(
	...
	*/
	...
	if int64(len(data)) > config.WhatsappSettingMaxDownloadSize {
		return response, fmt.Errorf("downloaded media size %d exceeds the maximum limit of %d bytes", len(data), config.WhatsappSettingMaxDownloadSize)
	}
